...
export default function SeccionBiblica({ section, pageValue }: SeccionBiblicaProps) {
            ...
            <div className='flex items-center flex-col mt-10 bg-orange-200 h-auto pb-4 xl:w-2/3 w-11/12 rounded-2xl border-opacity-30 shadow-md border-logo border-2'>
                <div className='bg-orange-100 bg-opacity-70 mt-7 mb-7 h-1/5 w-full flex flex-row items-center pl-7'>
                    ...
                    <div className='ml-5 flex flex-col gap-2 h-auto py-5'>
                        <p className='opacity-50 font-sans xl:text-base text-sm'>Seccion {section.id}</p>
                        <h2 className='xl:text-2xl text-logo font-semibold opacity-90 text-xl'>{section.title}</h2>
                        <p className='opacity-65 xl:text-base text-sm xl:text-start pr-1'>{section.description}</p>
                    </div>
                </div>
                ...
                <p className='text-sm mt-4 opacity-50'>Total de Recursos: {countPdf()} PDFs</p>
            </div>
        )
    }
}
